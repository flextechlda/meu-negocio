import { IInvoiceCart, IProduct, ISale } from "@/interfaces";
import { GetProductByID, GetSaleByID } from "@/models";
...
export const generate_sale_pdf = async (cartItems: IInvoiceCart[]) => {
    let productsHtmlContent = ``;

    for (let index = 0; index < cartItems.length; index++) {
        let saleId = cartItems[index]?.sale_id;
        let sale_data = await GetSaleByID({ sale_id: saleId });
        let product_data = await GetProductByID({
            product_id: sale_data?.product_id,
        });

        let _productsHtmlContent = await GetProductHtml(
            product_data,
            sale_data
        );
        productsHtmlContent = productsHtmlContent + _productsHtmlContent;
    }
    ...
    <html>
        ...
        <body>
            <h3 style="text-align: center">
                Relatório de Venda - ${formatDate(new Date().toString())} 
            </h3>
            <br/>
            <br/>
            <br/>
